<template>
  <page-header-wrapper>
    <template v-slot:content>
      ...
      <a-row>
        <a-button icon="plus" type="primary" @click="addPermission()">
          新增
        </a-button>
      </a-row>
    </template>
    ...
    <add-permission-modal :visible.sync="visible" @updateList="updateList"/>
    <update-permission-modal :updateVisible.sync="updateVisible" :item="item" @updateList="updateList"/>
  </page-header-wrapper>
</template>
  ...
  methods: {
    ...
    deleteItem (id) {
      this.$confirm({
        ...
        content: '是否要删除？',
        onOk: () => {
          this.$store.dispatch('DeletePermission', { id }).then(() => {
    ...
    getList (pagination) {
      ...
      this.loading = true
      this.$store.dispatch('SearchPermission', params).then(res => {
